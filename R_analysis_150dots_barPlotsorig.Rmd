...
library( boot )
dataRawA <-read.csv("Stats/mediateAllCSDAcc.csv", header=FALSE, stringsAsFactors = FALSE)
dataRaw <- read.csv("Stats/mediateAllCSD.csv", header=FALSE, stringsAsFactors = FALSE)
dataRawG<-read.csv("Stats/mediateAllGs.csv", header=FALSE, stringsAsFactors = FALSE)
...
data$RT=data$RT*2
data<-data%>% filter(Accuracy!="miss")
...
data<-dataA %>% #make factors
  mutate(side= ifelse(side==1,"Left","Right"),
         c= ifelse(c==2,"DP","DA"),
         hand = ifelse(hand==1,"left","right"),
         hit = ifelse(is.na(hit),"miss", ifelse(hit==1,"hit","wrong")))%>% #rename
  rename(., ID=subjectAll, Accuracy = hit, Hemi=side, distractor=c) %>%
  mutate_each_(funs(factor),c("distractor","Hemi","iti","ID","hand")) #make factor
testdata <- data %>% group_by(ID,distractor) %>% 
  mutate(hitwronmiss = ifelse(Accuracy=="hit",1,(ifelse(Accuracy=="miss",0,1))),
         totalTrials = sum(hitwronmiss))%>% filter(Accuracy!="miss")
#%>%
#  filter(Accuracy=="hit")
...
ANOVA_data <- AccID %>% group_by(ID,distractor,Accuracy) %>%
    select(ID, distractor,pect,Accuracy) %>%
    na.omit() %>%
    gather(key, dv, -ID,-distractor,-Accuracy)%>%filter(Accuracy=="hit")
...
#ANOVA_data2<-ANOVA_data%>%mutate(accuraydist = interaction(Accuracy,distractor))
#aov_accuraydist<-aov(dv ~ accuraydist + Error(ID/accuraydist), data=ANOVA_data2)
#pairs(emmeans(aov_accuraydist,~accuraydist),adjust="bonferroni")
```
...
png(savefile,width=400,heigh=350, units="px")
Accuracy_part <- AccID %>% group_by(ID,distractor,Accuracy)%>% summarise(meanpercT  = mean(pect , na.rm=TRUE))
Accuracy_part_summary<- Accuracy_part %>% group_by(distractor,Accuracy ) %>% summarise(sepercT  = se(meanpercT),meanpercT  = mean(meanpercT , na.rm=TRUE))

plotbehaviourHit <- Accuracy_part_summary %>% filter(Accuracy=="hit") %>% ggplot()+aes(x=distractor, y=meanpercT, fill=distractor,color=distractor) +
...
data<-dataA %>% #make factors
  mutate(side= ifelse(side==1,"Left","Right"),
         c= ifelse(c==2,"DP","DA"),
         hand = ifelse(hand==1,"left","right"),
         hit = ifelse(is.na(hit),"miss", "hit"))%>% #rename
  rename(., ID=subjectAll, Accuracy = hit, Hemi=side, distractor=c) %>%
  mutate_each_(funs(factor),c("distractor","Hemi","iti","ID","hand")) #make factor
testdata <- data %>% group_by(ID,distractor) %>% 
  mutate(hitwronmiss = ifelse(Accuracy=="hit",1,1),
...
AccID<-testdata%>% group_by(ID,distractor,Accuracy,totalTrials) %>%
                  summarise(totalNo  = sum(hitwronmiss, na.rm=TRUE))%>%
                  mutate(pect = as.numeric(totalNo/totalTrials*100))%>%filter(Accuracy=="miss")
ANOVA_data <- AccID %>% group_by(ID,distractor,Accuracy) %>%
    select(ID, distractor,pect,Accuracy) %>%
    na.omit() %>%
    gather(key, dv, -ID,-distractor,-Accuracy)
ACC_ANOVA <- ezANOVA(data = ANOVA_data
                        , dv = .(dv)
                        , wid = .(ID)
                        , within = .(distractor)
...
```
```{r plot results}
savefile<-file.path(figpaths,"IncorrectResponse.png")
png(savefile,width=400,heigh=350, units="px")
Accuracy_part_summary<- AccID %>% group_by(distractor ) %>% summarise(sepercT  = se(pect),meanpercT  = mean(pect , na.rm=TRUE))

AccuracyMissWrong <- Accuracy_part_summary
plotbehaviourWrongMiss <- AccuracyMissWrong %>% ggplot()+aes(x=distractor, y=meanpercT, fill=distractor,color=distractor) +
     # geom_violin(position = position_dodge(0.9)) +
   # geom_boxplot(width=0.15,position = position_dodge(0.9),fatten=NULL)+
  geom_col(width=0.5,alpha=0.3, position=position_dodge(0.9))+
  #  geom_boxplot( width=0.15,fatten=NULL)+
  geom_pointrange(aes(ymin=meanpercT-sepercT,ymax=meanpercT+sepercT),position = position_dodge(0.9))+
    xlab("Distractor") + ylab("Miss Rates (%)") +
    theme(axis.title.x = element_text(face="bold", size=20),
          axis.text.x  = element_text(face="bold", angle=0,  size=18),
          axis.title.y = element_text(face="bold", size=20),
          axis.text.y  = element_text(face="plain", angle=0, size=18),
          ...
          panel.background = element_blank())+
    scale_x_discrete(labels=c("Wrong", "Miss"))+
