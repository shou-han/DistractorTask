...
library( boot )
dataRawA <-read.csv("Stats/mediateAll5binCSDAccold.csv", header=FALSE, stringsAsFactors = FALSE)
dataRaw <- read.csv("Stats/mediateAll5binCSDold.csv", header=FALSE, stringsAsFactors = FALSE)
dataRawG<-read.csv("Stats/mediateAll5binGsold.csv", header=FALSE, stringsAsFactors = FALSE)
...
knitr::include_graphics((rep('../analysisScript/Figures/10dotsMatlab/CPPxdist10dots.png')))
knitr::include_graphics((rep('../analysisScript/Figures/10dotsMatlab/CPPrxdist10dots.png')))
knitr::include_graphics((rep('../analysisScript/Figures/10dotsMatlab/CPP10dots.png')))
```
...
W <-0.00123    #Mauchly's W from ezANOVA
...
## plotting them
# violin graphs 
savefile<-file.path(figpaths,"CountDistractorZBins.png")
...
```{r counts}
savefile<-file.path(figpaths,"CountZBins.png")
png(savefile,width=400,heigh=350, units="px")
ANOVA_dataPlotCPP<-plevelAll%>% group_by(ID,distractor,bins) %>% 
