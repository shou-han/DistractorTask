...
nopermANOVA<-function(dataInterest,interestValue){
  ...
  distractData <- ezANOVA(data = distdata
                          ...
                          , observed = NULL
                          , type = 3)
  return(list(distMean,distractData))
}
...
dataRawGB<-read.csv("Stats/mediateGBsold.csv", header=FALSE, stringsAsFactors = FALSE)
dataA<-genData(dataRawA);
dataERP<-genData(dataRaw)
dataERPG<-genData(dataRawG)
dataBetaG<-genData(dataRawGB)
#library(plyr)
...
summary(mediation.res)
```
##  Distractor -> Beta slope ->RT
```{r Beta slope, echo=FALSE, warning=FALSE, comment=''}
mediationModel<-'
RT ~ b*BetarSlope + cp*c 
BetarSlope ~ a*c 
mediation:=a*b
direct:=cp
total := a*b +cp
'
mediation.res<-sem(mediationModel, data=dataBetaG, estimator="MLM")
summary(mediation.res)
```
