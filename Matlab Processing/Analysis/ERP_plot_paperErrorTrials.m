...
for s = 1:size(ssj,2)
    sT = ssj(s);
    if CSD
        load(['Data/ERPs/group_plots_erp_diff_CSD_1binsStats_35Hz_' oldnew{onew} '_' num2str(sT) '_' num2str(chn)],'RTs'...
            ,'chanlocs','t','tr','allBins','no_of_bins',...
            'beta_side','beta_r_side','beta_contra','betar_contra','STFT_time','STFT_timer','RT_side',...
            'betaslope','alpha_side','alpha_asym_side','alpha_t','alpha_tr','mediationsA')
    else
        load(['Data/ERPs/group_plots_erp_diff__1binsStats_35Hz_' oldnew{onew} '_' num2str(sT) '_' num2str(chn)],'RTs'...
            ,'chanlocs','t','tr','allBins','no_of_bins',...
            'beta_side','beta_r_side','beta_contra','betar_contra','STFT_time','STFT_timer','RT_side',...
    ...
    for bin=1:no_of_bins
        for d=1:length(cs)
            for cong=1
                c = cs(d,:);
                allstuff_a.RTs{s,d,bin} = [mediationsA.RT(mediationsA.hit==0 & mediationsA.c==c & mediationsA.RT>300/2 & mediationsA.RT<1500/2)]*2;
...
for c = [1 2]
    ...
    de_temp = [allstuff_a.RTs{:,c,:}];
      set(gca,'linewidth',1.5,'Fontsmoothing','on','FontSize',16,'xlim',[400,1600],'xtick',[400:200:1600],'ylim',[0,30],'ytick',[0:10:30],'TickDir','out');%,'ylim',[-1.5,0.5]);
...
legend boxoff
saveas(hf,['Figures\RT_all_bins_error' namestring{onew} '.png']);
% make bar graphs
for c=[1 2]
...
yticks(300:300:1200);
legend(hs,dLabels,'FontSize',12,'Location','NorthEast','fontweight','bold')
saveas(hf,['Figures\RT_bar_graphs_error' namestring{onew} '.png']);
%% N2 graphs
%shadedErrorBar
%N2pc
clear hf hss1
...
for bin=1:1
    for c=1:2
     ...
     %       {'LineWidth',3,'LineStyle','-','Color',[ColsBeta(2+(bin-1)*2,:) alphaRT(c)]}),      
        set(gca,'linewidth',1.5,'Fontsmoothing','on','FontSize',16,'xlim',[-100 1400],'ytick',[-2:0.4:0.6],'xtick',[-100, 0:200:1400],'XTickLabel',xlabels_plot,'ylim',[-0.8 0.5],'TickDir','out');%'ylim',[0.5 0.8]);%,'ylim',[0.5,0.8]);
...
for bin=1:1
    for c=1:2
       ...
       % ipsi_beta_grp = squeeze(mean(betar_side_all(:,c,25,:,:),5));
        ...
        %    {'LineWidth',3,'LineStyle','-','Color',[ColsBeta(2+(bin-1)*2,:) alphaRT(c)]}),      
        set(gca,'linewidth',1.5,'Fontsmoothing','on','FontSize',16,'xlim',[-600 100],'ytick',[-0.8:0.2:0.4],'xtick',-600:200:100,'ylim',[-0.8 0.4],'TickDir','out');%'ylim',[0.5 0.8]);%,'ylim',[0.5,0.8]);
        ylabel('Beta (\muVolts)','FontName','Arial','FontSize',16,'fontweight','bold')
        xlabel('Time from Response (ms)','FontName','Arial','FontSize',16,'fontweight','bold')
       % title('Contra Beta x Distractor xRT')
        line([0,0],ylimBeta,'Color','k','LineWidth',3.0,'LineStyle','-');
