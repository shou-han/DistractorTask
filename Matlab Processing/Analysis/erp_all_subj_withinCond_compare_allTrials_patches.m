...
% save all the plots in a group and the plot can choose whichever one it wants
% participant 9, choose a different set of electrodes
function erp_all_subj_withinCond_compare_allTrials_patches(number)
%number=4;
disp('start')
oldnew = {'', 'old'};
oldAll=[2];
Hzused=[8 35];
populations=1:21;
[a, b, c] = ndgrid(populations,Hzused,oldAll);
allcombinations = [a(:) b(:) c(:)];
conditions = allcombinations(number,:);
single_participants = conditions(1); Hzused=conditions(2);old=conditions(3);jobID=number;
...
skip_step = 10;
%%
side_tags = {'Congruent','Incongruent'};
...
%% patches congurent/incongruent with hand
targcodes(:,:,1) =   [101   103   117   119   133   135 110   112   126   128   142   144;...
    105   107   121   123   137   139 113   115   129   131   145   147];
targcodes(:,:,2) =   [ 109   111   125   127   141   143 102   104   118   120   134   136;...
    114   116   130   132   146   148 106   108   122   124   138   140];
...
for s=1:length(single_participants)
    ...
    if Hzused==8
        if CSD
            erp_all=double(erp_LPF_8Hz_CSD); %20Hz
            erp_beta=double(erp_LPF_8Hz_CSD); %35Hz since looking at beta
        else
            erp_all=double(erp_LPF_8Hz);
            erp_beta=double(erp_LPF_8Hz); %35Hz since looking at beta
        end
    end
    if Hzused==35
        if CSD
    ...
    for trial=1:size(erp,3)
        ...
        erpN(left_hemi,:,trial) = erp(ch_contra(cong,:),:,trial); %save as new variable since transformation problems
        erpN(right_hemi,:,trial) = erp(ch_ipsi(cong,:),:,trial);
    ...
    beta_TSE_base = STFT-repmat(baseline_beta,[1,size(STFT,2),1]); % baseline full beta
    ...
    if CSD
        window_searchi = [150,500];
        window_searchc = [150 500];
        window_searchpc = [150 500];
    ...
    for d = 1:size(cs,1)
        for cong=1:2
            ...
            c_ind =  find(ismember(allTrig,squeeze(targcodes(c,:,cong))));
            ...
            for cong = 1:2
                for iti = 1:size(targcodes,2)
                    ...
                    conds{s,d,iti,cong} = find(ismember(allTrig,targcodes(c,iti,cong)) & ~isnan(allacc) &...
                    ...
                    condsACC{s,d,iti,cong} = find(ismember(allTrig,targcodes(c,iti,cong)) & ET_BL_resp_artrej ); %
                    ...
                    condsT{s,d,iti,cong} = find(ismember(allTrig,targcodes(c,iti,cong)) );
                    RTs{s,d,iti,cong} = allRT([conds{s,d,iti,cong}])*1000/fs;
                end
            end
        end
    end
    ...
    for d=1:size(cs,1)
        for cong=1:2
            for iti=1:size(targcodes,2)
                RT_temp = [RTs{s,d,iti,cong}];
                cond_temp=[conds{s,d,iti,cong}];
                ...
                for bin =  [1:binch-1]
                    ...
                    conds_bin{s,d,iti,cong,bin} = conds_all;
                ...
                conds_bin{s,d,iti,cong,binch} = conds_all;
    ...
    disp(['Subject ',allsubj{s},' Condition Valid Trials: ',num2str(length([conds{s,:,:,:,:}])), ...
        ' = ',num2str(round(100*(length([conds{s,:,:,:,:}]))/length([condsT{s,:,:,:}]))),'%'])
    clear beta_asym_bins RT_bins RT_bins_all
    ...
    for c=1:size(cs,1)
        for bin=1:no_of_bins
            for cong=1:2
                ...
                CPP_mean_temp = cat(2, CPP_mean_temp, squeeze(mean(erp(ch_CPP,:,[conds_bin{s,c,:,cong,bin}]),1)));
                CPPr_mean_temp = cat(2, CPPr_mean_temp, squeeze(mean(erpr(ch_CPP,:,[conds_bin{s,c,:,cong,bin}]),1)));
                for k = [conds_bin{s,c,:,cong,bin}]
                    % find slope average
                    CPPslope_mean_temp (:,kk) = movingslope(mean(erp(ch_CPP,:,k),1),200,4);
                    CPPrslope_mean_temp (:,kk) = movingslope(mean(erpr(ch_CPP,:,k),1),200,4);
                    kk=kk+1;
                end
                N2c_mean_temp = squeeze(erp(ch_lr,:,[conds_bin{s,c,:,cong,bin}]));
                N2i_mean_temp = squeeze(erp(ch_rl,:,[conds_bin{s,c,:,cong,bin}]));
                N2pc_mean_temp= squeeze(erp(ch_lr,:,[conds_bin{s,c,:,cong,bin}])-erp(ch_rl,:,[conds_bin{s,c,:,cong,bin}]));
                RT_temp = allRT([conds_bin{s,c,:,cong,bin}]);
                erp_mean_temp = squeeze(erp(:,:,[conds_bin{s,c,:,cong,bin}]));
                erpr_mean_temp = squeeze(erpr(:,:,[conds_bin{s,c,:,cong,bin}]));
                
                % store the graphs for plotting
                allBins.Rts{s,c,cong,bin} = allRT([conds_bin{s,c,:,cong,bin}])*1000/fs;
                allBins.ERP_side(s,c,:,:,cong,bin) = squeeze(mean(erp_mean_temp,3));
                allBins.ERPr_side(s,c,:,:,cong,bin) = squeeze(mean(erpr_mean_temp,3));
                
                allBins.CPP_side(s,c,:,cong,bin) = squeeze(mean(CPP_mean_temp,2));
                allBins.CPPr_side(s,c,:,cong,bin) = squeeze(mean(CPPr_mean_temp,2));
                allBins.CPPslopes_side(s,c,:,cong,bin) = squeeze(mean(CPPslope_mean_temp,2));
                allBins.CPPrslopes_side(s,c,:,cong,bin) = squeeze(mean(CPPrslope_mean_temp,2));
                
                allBins.N2c_side(s,c,:,cong,bin) = squeeze(mean(N2c_mean_temp,2));
                allBins.N2i_side(s,c,:,cong,bin) = squeeze(mean(N2i_mean_temp,2));
                allBins.N2pc_side(s,c,:,cong,bin) = squeeze(mean(N2pc_mean_temp,2));
                
                % Betas
                beta_r_side(s,c,:,:,cong,bin) = squeeze(mean(STFTCIr(1:numch,:,[conds_bin{s,c,:,cong,bin}]),3)); %(s, chan, STFT_timers, targetside)
                beta_side(s,c,:,:,cong,bin) = squeeze(mean(beta_CI_base(1:numch,:,[conds_bin{s,c,:,cong,bin}]),3));
                % asymmetry betas
                beta_asym_side(s,c,:,:,cong,bin) = squeeze(mean(beta_asym(size(beta_asym,1),:,[conds_bin{s,c,:,cong,bin}]),3));
                
                beta_contra(s,c,:,cong,bin) = squeeze(mean(mean(beta_CI_base(ch_beta_ci(1,:),:,[conds_bin{s,c,:,cong,bin}]),3),1));
                betar_contra(s,c,:,cong,bin) = squeeze(mean(mean(STFTCIr(ch_beta_ci(1,:),:,[conds_bin{s,c,:,cong,bin}]),3),1));
                
                beta_ipsi(s,c,:,cong,bin) = squeeze(mean(mean(beta_CI_base(ch_beta_ci(2,:),:,[conds_bin{s,c,:,cong,bin}]),3),1));
                betar_ipsi(s,c,:,cong,bin) = squeeze(mean(mean(STFTCIr(ch_beta_ci(2,:),:,[conds_bin{s,c,:,cong,bin}]),3),1));
                
                % collect all trials
                beta_contra_all{s,c,cong,bin} = mean(beta_CI_base(ch_beta_ci(1,:),:,[conds_bin{s,c,:,cong,bin}]),1);
                betar_contra_all{s,c,cong,bin} = mean(STFTCIr(ch_beta_ci(1,:),:,[conds_bin{s,c,:,cong,bin}]),1);
                beta_ipsi_all{s,c,cong,bin} = mean(beta_CI_base(ch_beta_ci(2,:),:,[conds_bin{s,c,:,cong,bin}]),1);
                betar_ipsi_all{s,c,cong,bin} = mean(STFTCIr(ch_beta_ci(2,:),:,[conds_bin{s,c,:,cong,bin}]),1);
                kk=1;
                % beta slopes
                for k = [conds_bin{s,c,:,cong,bin}]
                    % find slope average
                ...
                betaslope.contra(s,c,:,cong,bin) = squeeze(mean(betac_slope_temp,2));
                betaslope.contraR(s,c,:,cong,bin) = squeeze(mean(betarc_slope_temp,2));
                betaslope.ipsi(s,c,:,cong,bin) = squeeze(mean(betai_slope_temp,2));
                betaslope.ipsiR(s,c,:,cong,bin)  = squeeze(mean(betari_slope_temp,2));
                
                %
                RT_side{s,c,cong,bin} = allRT([conds_bin{s,c,:,cong,bin}])*1000/fs;
                
                RT_st_data{c,cong,bin} = RT_side{s,c,cong,bin};
                
                
                
                alpha_side(s,c,:,:,cong,bin) = squeeze(mean(alpha_TSE(:,:,[conds_bin{s,c,:,cong,bin}]),3));
                alpha_asym_side(s,c,:,:,cong,bin) = squeeze(mean(alpha_asym(:,:,[conds_bin{s,c,:,cong,bin}]),3));
                ...
                [mediations.N2c_onset(j)] = obtainOnset_final_subj(-N2c_mean_temp,...
                    window_searchc,15,t,0); %the onset will have the same fields as ERPs
                [mediations.N2i_onset(j)] = obtainOnset_final_subj(-N2i_mean_temp,...
                    window_searchi,15,t,0); %the onset will have the same fields as ERPs
    ...
    for c=1:size(cs,1)
        for cong=1:2
            for iti=1:size(targcodes,2)
                if ~isempty(condsACC{s,c,iti,cong})
                    for jj=1:size(condsACC{s,c,iti,cong},2)
                        k=k+1;
                        kk = condsACC{s,c,iti,cong}(jj);
    ...
    for c=1:size(cs,1)
        for bin=1:no_of_bins
            for cong=1:2
                for iti=1:size(targcodes,2)
                    if ~isempty(conds_bin{s,c,iti,cong,bin})
                        %size(conds_bin{s,c,iti,side,bin},2)
                        for jj=1:size(conds_bin{s,c,iti,cong,bin},2)
                            k=k+1;
                            kk = conds_bin{s,c,iti,cong,bin}(jj);
                            ...
                            % find the CPP at RT
                            mediationsS.CPPlevel(k) = mean(CPPrTemp(tr>-100 & tr<100));
                            ...
                            cpponsetSlope = mediations.CPPonset;
                            if sum(cpponsetSlope==5000)>0, cpponsetSlope(cpponsetSlope==5000)=0,end;
                            windowS_slopeEarly=nanmean(cpponsetSlope) + window_slopeStimEarly;
                            windowS_slopeLate= nanmean(cpponsetSlope) + window_slopeStimLate;
                            ...
                            % is going down
                            mediationsSB.BetaLevel(k) = mean(BetaTemp(STFT_timer>-100 & STFT_timer<100));
                            clear tindx onsetIndx
    ...
    for c=1:size(cs,1)
        c
        clear indx
        indx = indxA(mediationsM.c(indxA)==c); bin=0;binA=0;       grp = floor(length(indx)/no_of_bins);
        for i=1:grp:length(indx)-rem(length(indx),grp)
            clear condsbinc
            bin= bin+1;binA=binA+1; %binA is an index for all the bins
            %         for f=length(mms)-1:length(mms)
...
if CSD
    save(['Data/ERPs/group_plots_erp_diff_CSD_1binsStats_cong_' num2str(Hzused) 'Hz_allTrials_' oldnew{old+1} '_' num2str(single_participants) '_' num2str(chP)],'RTs','t','plot_chans','tr','chanlocs',...
    ...
    save(['Data/ERPs/group_plots_erp_diff_1binsStats_cong_' num2str(Hzused) 'Hz_allTrials__unbaselinedBeta' oldnew{old+1} '_' num2str(single_participants) '_' num2str(chP)],'RTs','t','plot_chans','tr','chanlocs',...
        'allBins','allsubj','subject_folder', 'allstuff','no_of_bins','mediations','mediationsS','mediationsA','mediationsG','mediationsSA',...
        'conds','condsACC','conds_bin',...
        'beta_side','beta_r_side','STFT_time','STFT_timer',...
        'beta_contra','betar_contra','beta_ipsi','betar_ipsi','RT_side','beta_hemi_diff','betar_hemi_diff',...
