...
for s=1:size(allsubj,2)
    ...
    if CSD
        load(['Data/ERPs/group_plots_erp_diff_CSD_1binsStats_cong_8Hz_allTrials_' oldnew{onew} '_' num2str(sT) '_' num2str(chn)],'mediations','mediationsS','mediationsA','chanlocs','t','tr')
        load(['Data/ERPs/group_plots_erp_diff_CSD_1binsStats_cong_35Hz_allTrials_' oldnew{onew} '_' num2str(sT) '_' num2str(chn)],'mediationsSB','mediationsGB','STFT_time','STFT_timer','mediationsSA','mediationBeta','mediationsG')
    else
...
%%
if CSD
    struct2csv (mediationAll,[patht 'mediateAllcongCSD' oldnew{onew} '.csv']);
    struct2csv (mediationAllA,[patht 'mediateAllcongCSDAcc' oldnew{onew} '.csv']);
    struct2csv (mediationGs,[patht 'mediateAllcongGs' oldnew{onew} '.csv']);
    struct2csv (mediationGBs,[patht 'mediateAllcongGBs' oldnew{onew} '.csv']);
