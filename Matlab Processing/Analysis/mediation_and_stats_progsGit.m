...
for s=1:size(allsubj,2)
    ...
    if CSD
        load(['Data/ERPs/group_plots_erp_diff_CSD_1binsStats_uB_8Hz_allTrials_' oldnew{onew} '_' num2str(sT) '_' num2str(chn)],'mediations','mediationsS','mediationsA','chanlocs','t','tr')
        load(['Data/ERPs/group_plots_erp_diff_CSD_1binsStats_uB_35Hz_allTrials_' oldnew{onew} '_' num2str(sT) '_' num2str(chn)],'mediationsSB','mediationsGB','STFT_time','STFT_timer','mediationsSA','mediationBeta','mediationsG')
    else
...
%%
if CSD
    struct2csv (mediationAll,[patht 'mediateAlluBCSD' oldnew{onew} '.csv']);
    struct2csv (mediationAllA,[patht 'mediateAlluBCSDAcc' oldnew{onew} '.csv']);
    struct2csv (mediationGs,[patht 'mediateAlluBGs' oldnew{onew} '.csv']);
    struct2csv (mediationGBs,[patht 'mediateAlluBGBs' oldnew{onew} '.csv']);
    save(['slopeDataIndividuBCSD' oldnew{onew} '.mat'],  'CPPslopeDA', 'CPPslopeDP', 'CPPrslopeDA', 'CPPrslopeDP',  't', 'tr', 'mediationAllG', 'mediationAllGB', 'STFT_time', 'STFT_timer')
else
