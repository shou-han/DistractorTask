function [totalOnsetOut] = obtainOnset_old_subj( data, ch_CPP, conds, winsmth, consecutive_windows, totalOnsetOut,...
    c,side,t)
%% find onset CPP
...
for flds = 1:1
    ...
    for trial = 1:size(CPP_temp,2)
        ...
        win_mean1(:,trial)=win_mean(:,trial);
        %win_mean(:,trial) = abs(win_mean(:,trial));
    end
    
    % do t-test to zero across the smoothed trials.
    for tt = 1:size(win_mean,1)
        ts = 451-tt+1; %reverse it
        [~,P,~,STATS] = ttest2(win_mean(ts,:),0*win_mean(ts,:),'tail','right');
    ...
    for i = 1:length(allp05)
        if  (i+consecutive_windows-1)<=length(allp05)
            if allp05(i+consecutive_windows-1)-allp05(i)==consecutive_windows-1 %if there is at least 10 consecutive windows where p>.05
                onsetp05=allp05(i+consecutive_windows-1);%take last point
                break
            end
        else
            onsetp05=allp05(end);%take last point
            break
        end
    end
    
    % get timepoint of min index.
    if ~isempty(onsetp05) && onsetp05<451
        onset_ind = win_mean_inds(onsetp05); %from 0 to 1000 window
        CPP_onset_ind = 451-onset_ind+1;
        onsetOut(flds) = t(CPP_onset_ind-1);
 ...
 %   plot(tr(CPP_onset_ind), mean(win_mean(CPP_onset_ind,:),2),'r+','Markersize',20)
 %   hold on
 %   plot(tr(1:451), mean(win_mean(:,:),2))
