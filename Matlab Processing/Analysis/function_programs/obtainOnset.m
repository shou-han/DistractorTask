function [totalOnsetOut, totalAmpIndx, totalAmp] = obtainOnset( data, conds, CPP_search_t,...
    t,maxT,max_search_window,...
ptS, consecutive_windows, allsubj, ch_CPP,s,c,side, totalOnsetOut, totalAmpIndx, totalAmp, resplocked)
%% find onset CPP
...
%         onsetOut(flds)
%         max(t)
        for tt = 1:size(dtemp,2)
            temp2 = dtemp(find(t>onsetOut(flds),1):find(t==maxT),tt);
            maxAmp{flds}(tt) = max(temp2);
            ampIndex{flds}(tt) = find( temp2 == maxAmp{flds}(tt),1)+CPP_onset_ind;
            clear temp2
        end
    else % no trials
        for tt = 1:1
            ampIndex{flds}(tt) = CPP_onset_ind; %no amplitude
            maxAmp{flds}(tt) = 0;
        end
%              disp([allsubj,': bugger'])
%              pause
...
%     figure(1101)
%     hold on
%     ampM = floor(mean([ampIndex{flds}]));
%     maxA = (mean([maxAmp{flds}]));
%     plot(t,mean(dU,2))
