function [powerLevel] = obtainPower( data_used, t, RT_used,resplocked,flag)
...
if ~resplocked
    for trial = 1:size(data_used,2)
        ...
        powerLevelTemp(trial) = norm(sigDiffData)/RT_used(trial);
    end
...
if strcmp(flag,'sum')
powerLevel = sum(powerLevelTemp);
end
if strcmp(flag,'mean')
powerLevel = mean(powerLevelTemp);
end
% plotting
% hold on
% trial = 34;
